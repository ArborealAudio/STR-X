...
pub const StrY = struct {
    ...
    pub fn init(arena: Allocator, num_ch: u32) !*Processor {
        const self: *StrY = try arena.create(StrY);
        ...
        self.* = .{
            ...
            .params = params,
            .preamp = PreAmp.init(params, arena, num_ch) catch {
                return error.PreAmpInitFailed;
            },
            .tone_stack = ToneStack.init(params, arena, num_ch) catch {
                return error.ToneStackInitFailed;
            },
            .poweramp = PowerAmp.init(params, arena, num_ch) catch {
    ...
    fn prepare(
        ...
        const self: *StrY = @fieldParentPtr("proc", proc);
    ...
    fn reset(p: *Processor) void {
        const self: *StrY = @fieldParentPtr("proc", p);
        self.preamp.reset();
        self.poweramp.reset();
    }
    ...
    const PreAmp = struct {
        ...
        pub fn init(state: *const Params, arena: Allocator, num_ch: u32) !PreAmp {
            return .{
                .state = state,
                .hi_filter = try Filter.init(arena, num_ch, .FirstOrderHighpass, 600, 0),
                .low_filter = try Filter.init(arena, num_ch, .FirstOrderLowpass, 2500, 0),
                .dc_removal = try Filter.init(arena, num_ch, .Highpass, 10, std.math.sqrt1_2),
            };
        }

        fn prepare(self: *PreAmp, sample_rate: f64, _: u32) void {
            self.hi_filter.setSampleRate(@floatCast(sample_rate));
            self.low_filter.setSampleRate(@floatCast(sample_rate));
            self.dc_removal.setSampleRate(@floatCast(sample_rate));
        }

        fn reset(self: *PreAmp) void {
            self.hi_filter.reset();
        ...
        fn processHiGain(self: *PreAmp, buffer: AudioBuffer) void {
            const lg = self.state.low_gain * 4;
            const hg = self.state.hi_gain * 4;

            for (buffer.data, 0..) |ch, ch_idx| {
                for (ch) |*sample| {
                    var y: f32 = sample.*;

                    var yh = self.hi_filter.processSample(ch_idx, y);
                    var yl = self.low_filter.processSample(ch_idx, y);
        ...
        fn processLoGain(self: *PreAmp, buffer: AudioBuffer) void {
            const lg = self.state.low_gain * 2;
            const hg = self.state.hi_gain * 2;
            for (buffer.data, 0..) |ch, ch_idx| {
                for (ch) |*sample| {
                    var y = sample.*;
