...
public:
	...
	void updateCrossover(float crossover)
	{
	...
	double processAudioSample(double xn, float inputGain)
	{
		float gain = inputGain * 8;
		double yn = 0.0;
		float k = inputGain / 3;
		float negK = k / 3;
...
public:
	...
	double processAudioSample(double xn, float outGain)
	{
		float gain = outGain * 0.6;
		double yn = 0.0;
...
public:
	...
	void updateFilters(double sampleRate, float bassGain, float midGain, float trebleGain,
	...
	void processAmp(const dsp::ProcessContextReplacing<float>& context) noexcept
	{
